...
class OnlyS(AbsWeighting):
    ...
    """
    def __init__(self):
        super(OnlyS, self).__init__()
        
    def backward(self, losses, **kwargs):
        loss = torch.mul(losses, torch.from_numpy(np.array([1,0,0])).to(self.device)).sum()#torch.ones_like(losses).to(self.device)).sum()
